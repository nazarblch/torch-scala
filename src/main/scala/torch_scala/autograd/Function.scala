...
abstract class Function[T: ClassTag, TT <: TensorType](val name: String, args: Variable[_, TT]*) {

  def varName: String = args.length match {
    case 0 => name
    case 2 => s"(${args(0)} $name ${args(1)})"
  ...
  def backward(gradOutput: Tensor[T, TT]): Seq[(Variable[_, _], Tensor[_, _])] = {
    val grads = backwardImpl(gradOutput)
    require(args.length equals grads.length, "Each argument should have gradient")
    args.zip(grads)
  }

  protected def backwardImpl(gradOutput: Tensor[T, TT]): Seq[Tensor[_, _]]
...
case class Add[T: ClassTag, TT <: TensorType](v1: Variable[T, TT], v2: Variable[T, TT]) extends Function[T, TT]("+", v1, v2) {
  ...
  override def backwardImpl(gradOutput: Tensor[T, TT]): Seq[Tensor[_, _]]  = {
...
case class Sub[T: ClassTag, TT <: TensorType](v1: Variable[T, TT], v2: Variable[T, TT]) extends Function[T, TT]("-", v1, v2) {
  def forwardImpl(): Tensor[T, TT] =  v1.data - v2.data
  def backwardImpl(gradOutput: Tensor[T, TT]): Seq[Tensor[_, _]] = {
    val g1 = unbroadcast(gradOutput, v1.shape)
    val g2 = unbroadcast(-gradOutput, v2.shape)
    Seq(g1, g2)
  }
...
case class Sigmoid[T: ClassTag, TT <: TensorType](v: Variable[T, TT])(implicit num: Numeric[T]) extends Function[T, TT]("sigmoid") {
...
case class Softmax[T: ClassTag, TT <: TensorType](v: Variable[T, TT], dim: Long) extends Function[T, TT]("softmax") {
...
case class MeanByAxis[T: ClassTag, TT <: TensorType](v: Variable[T, TT], axis: Int)(implicit num: Numeric[T]) extends Function[T, TT]("mean") {
...
case class Variance[T: ClassTag, TT <: TensorType](v: Variable[T, TT])(implicit num: Numeric[T]) extends Function[T, TT]("var") {
  ...
  override def backwardImpl(gradOutput: Tensor[T, TT]): Seq[Tensor[_, _]] = {
    (v - v.mean()).pow(num fromInt 2).mean().backward()
    Seq(v.grad.data)
  }
}

case class VarianceByAxis[T: ClassTag, TT <: TensorType](v: Variable[T, TT], axis: Int)(implicit num: Numeric[T]) extends Function[T, TT]("var") {
...
case class Max[T: ClassTag, TT <: TensorType](x: Variable[T, TT], y: Variable[T, TT]) extends Function[T, TT]("max") {
...
case class Threshold[T: ClassTag, TT <: TensorType](x: Variable[T, TT], d: Scalar[T]) extends Function[T, TT]("threshold") {
...
case class SoftmaxLoss[T: ClassTag, TT <: TensorType](actual: Variable[T, TT], target: Variable[Long, TT])(implicit num: Numeric[T]) extends Function[T, TT]("softmaxLoss") {
  ...
  def backwardImpl(gradOutput: Tensor[T, TT] = null /* not used */ ): Seq[Tensor[_, _]]  = {
