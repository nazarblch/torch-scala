...
object ScalaMethodDeclaration {
  ...
  def toScalaAnnotation(ann: AnnotationExpr): String = {
    ...
    } else if((ann.getName.toString equals "ByRef") || (ann.getName.toString equals "ByVal")) {
       "@" + ann.getName
    } else {
      ann.toString
    }
  ...
  def toScalaType(name: String, methodName: String, valName: Option[String], templates: String): String = {
    ...
    name.trim match {
      ...
      case "TensorTupleAndDoubleLong<T,TT>" => "TensorTupleAndDoubleLong[T,TT]"
      case "TensorTripleAndLong<T,TT>" => "TensorTripleAndLong[T,TT]"
      case "TensorTuple4AndLong<T,TT>" => "TensorTuple4AndLong[T,TT]"
      ...
      case "ArrayRef<double>" => "ArrayRefDouble"
      case _ => name
